...
def home(): 
   ...
   # st.markdown(f"<h4>Hi {name}</h4>", unsafe_allow_html=True)
    ...
    #Happy Genre
    happy_mood =['dnb','hardstyle','RnB']
    sad_mood =['Emo','RnB','trance','psytrance']
    angry_mood = ['Hiphop','Dark Trap','Underground Rap','Trap Metal','Rap','trap']
    relaxed_mood = ['Rap','RnB','techhouse','techno','trance','psytrance','hardstyle']

    desired_genre = happy_mood
    # Retrieve songs based on genre
    if emotion == "Happy😍":
        desired_genre=happy_mood
    if emotion ==  "Sad🥺":
        desired_genre=sad_mood
    if emotion ==  "Angry😤":
        desired_genre=angry_mood
    if emotion == "Relaxed😊":
        desired_genre=relaxed_mood
    
    # Replace with the desired genre
    songs_by_genre = df[df["genre"].isin(desired_genre)]
    ...
    chunks = [song_urls[i:i + num_cols] for i in range(0, 20, num_cols)]
